...
import { CircuitString, Struct, UInt64 } from "o1js"
import { MatchRecords } from "../utils/matchRecords"

export class Team {
    teamName: CircuitString
    money: UInt64
    players: Player[]
    positionPairs: afterPositioning[]
    matchRecords : MatchRecords;

    constructor(teamName: CircuitString,
                money: UInt64 = UInt64.from(1000),
                matchRecords : MatchRecords = new MatchRecords(UInt64.from(0),UInt64.from(0),UInt64.from(0),UInt64.from(0),UInt64.from(0),UInt64.from(0),UInt64.from(0),UInt64.from(0)))
      {
        this.matchRecords = matchRecords; // A new team has all records of 0, of course
